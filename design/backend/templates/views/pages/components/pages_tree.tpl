            ...
            >
                    ...
                    {strip}
                        <div class="text-over" {if $search.get_tree == "multi_level"}style="padding-{$direction}: {math equation="x*14" x=$page.level|default:0}px;"{/if}>
                            {if $page.subpages || $page.has_children}
                                ...
                                {/if}
                                <a href="#" alt="{__("expand_sublist_of_items")}" title="{__("expand_sublist_of_items")}" id="on_page{$combination_suffix}_{$page.page_id}" class="link--monochrome cm-combination-pages{$combination_suffix} {if $expand_all && !$hide_show_all}hidden{/if}" {if $page.has_children}onclick="Tygh.$.ceAjax('request', '{"$_dispatch?parent_id=`$page.page_id`&get_tree=multi_level`$except_url`&display=`$display`&checkbox_name=`$checkbox_name`&combination_suffix=`$combination_suffix`{if $is_exclusive_page_type}&page_type={$page.page_type}{/if}"|fn_url nofilter}', {$ldelim}result_ids: 'page{$combination_suffix}_{$page.page_id}', caching: true{$rdelim});"{/if}><span class="flex-inline top">{include_ext file="common/icon.tpl" source="caret_right" class="cs-icon--small"}</span></a>
                                <a href="#" alt="{__("collapse_sublist_of_items")}" title="{__("collapse_sublist_of_items")}" id="off_page{$combination_suffix}_{$page.page_id}" class="link--monochrome cm-combination-pages{$combination_suffix} {if !$expand_all || $hide_show_all}hidden{/if}"><span class="flex-inline top">{include_ext file="common/icon.tpl" source="caret_down" class="cs-icon--small"}</span></a>
                            {elseif $search.get_tree == "multi_level"}
                                <span style="padding-{$direction}: 14px;">&nbsp;</span>
                            {/if}


                            {if !$picker}<a href="{"pages.update?page_id=`$page.page_id`&come_from=`$_come_from`"|fn_url}" class="link--monochrome {if $page.status == "N"}manage-root-item-disabled{/if}" id="page_title_{$page.page_id}" title="{$page.page}">{else}<label class="inline-label" for="radio_{$page.page_id}" id="page_title_{$page.page_id}">{/if}
                                {$page.page}
                ...
                {if !$hide_delete_button}
                    <td width="10%" class="nowrap right" data-th="{__("status")}">
                        {if "ULTIMATE"|fn_allowed_for && $allow_save || !"ULTIMATE"|fn_allowed_for}
                            {include file="common/select_popup.tpl" type="pages" id=$page.page_id status=$page.status hidden=true object_id_name="page_id" table="pages" popup_additional_class="dropleft"}
                        {/if}
                    </td>
                {/if}
            </tr>
