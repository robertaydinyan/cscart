...
{capture name="adv_buttons"}
    ...
    {if !$filter_fields && !$filter_features}
        {$add_filter_button_meta = "btn-primary cm-disabled disabled" }
        {$add_filter_button_tooltip = __("filters_in_use") }
    {else}
        {$add_filter_button_meta = "btn-primary" }
        {$add_filter_button_tooltip = __("add_filter") }
    {/if}
    {include file="common/popupbox.tpl"
        id="add_product_filter"
        text=__("new_filter")
        content=$smarty.capture.add_new_picker
        title=$add_filter_button_tooltip
        link_text=__("add_filter")
        act="general"
        icon="icon-plus"
        link_class=$add_filter_button_meta
    }
    {/if}
