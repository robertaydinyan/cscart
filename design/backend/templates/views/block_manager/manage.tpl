...
{capture name="buttons"}
    {if $location.dispatch === "products.view"}
        {include file="buttons/button.tpl" but_text=__("actions.product_tabs") but_role="action" but_href="tabs.manage"}
    {/if}
        ...
        {foreach from=$navigation.tabs item=tab key=key name=tabs}
                <li id="{$key}{$id_suffix}" class="{if $tab.hidden == "Y"}hidden {/if}{if $key == "location_`$location.location_id`"}active extra-tab{/if}">
                    {if $key == "location_`$location.location_id`" && $show_layout_controls}
                        {btn type="dialog" class="cm-ajax-force hand icon-cog" href="block_manager.update_location?location=`$location.location_id`&s_layout=`$location.layout_id`" id="tab_location_`$location.location_id`" title=$tab.title icon="icon-cog"}
                    {/if}
