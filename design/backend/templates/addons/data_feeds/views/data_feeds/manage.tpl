        ...
        <div class="table-responsive-wrapper longtap-selection">
            ...
            >
                ...
                <td width="45%" data-th="{__("name")}">
                    <a href="{"data_feeds.update?datafeed_id=`$datafeed.datafeed_id`"|fn_url}" class="link--monochrome">{$datafeed.datafeed_name}</a>
                ...
                <td width="8%" class="nowrap right" data-th="{__("status")}">
                    {include file="common/select_popup.tpl" type="data_feeds" id=$datafeed.datafeed_id status=$datafeed.status hidden=false object_id_name="datafeed_id" table="data_feeds"}
                </td>
...
{capture name="adv_buttons"}
    {include file="common/tools.tpl"
        tool_href="data_feeds.add"
        tool_override_meta="btn btn-primary"
        prefix="bottom"
        title=__("add_datafeed")
        link_text=__("add_datafeed")
        hide_tools=true
        icon="icon-plus"
    }
{/capture}
