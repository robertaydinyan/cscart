        ...
        {capture name="store_locator_table"}
            <div class="table-responsive-wrapper longtap-selection">
                ...
                <tr>
                    ...
                    <th width="20%">{__("store_locator")}</th>
                    ...
                    >
                        ...
                        <td width="20%" data-th="{__("store_locator")}">
                            <a class="row-status link--monochrome" href="{"store_locator.update?store_location_id=`$loc.store_location_id`"|fn_url}">{$loc.name}</a>
                            ...
                            {if $loc.main_destination_id}
                                ...
                                {if fn_check_view_permissions("destinations.update")}
                                    <a href="{"destinations.update&destination_id={$loc.main_destination_id}"|fn_url}"
                                       class="row-status link--monochrome {if $destinations[$loc.main_destination_id]['status'] === "ObjectStatuses::DISABLED"|enum}shipping-rate-area-d{/if}"
                        ...
                        <td width="10%" class="right nowrap" data-th="{__("status")}">
                            {include file="common/select_popup.tpl" type="store_locations" id=$loc.store_location_id status=$loc.status hidden="" object_id_name="store_location_id" table="store_locations" popup_additional_class="`$no_hide_input`" display=$display}
                        </td>
...
<!--store_locations--></div>
    ...
    {capture name="adv_buttons"}
        {include file="common/tools.tpl"
            tool_href="store_locator.add"
            tool_override_meta="btn btn-primary"
            prefix="top"
            title=__("add_store_location")
            link_text=__("add_store_location")
            hide_tools=true
        }
    {/capture}
