                            ...
                            {foreach $threads as $thread}
                                ...
                                >
                                    ...
                                    <td width="14%" class="{if $has_new_message}status-new__text{/if}" data-th="{__("id")}">
                                        <a href="{$thread_href}" class="link--monochrome">
...
{capture name="adv_buttons"}
    {$_title=__("vendor_communication.message_center")}

    {if $communication_type === "Addons\\VendorCommunication\\CommunicationTypes::VENDOR_TO_CUSTOMER"|enum}
        {$_title = "`$_title`: {__("customers")}"}
    {elseif $communication_type === "Addons\\VendorCommunication\\CommunicationTypes::VENDOR_TO_ADMIN"|enum && $runtime.company_id}
        {$_title = "`$_title`: {__("administrator")}"}
    {elseif $communication_type === "Addons\\VendorCommunication\\CommunicationTypes::VENDOR_TO_ADMIN"|enum && !$runtime.company_id}
        {$_title = "`$_title`: {__("vendors")}"}
    {/if}
{/capture}
