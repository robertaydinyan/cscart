        ...
        <div class="table-responsive-wrapper longtap-selection">
            ...
            >
                ...
                <td class="{$no_hide_input}" data-th="{__("banner")}">
                    <a class="row-status link--monochrome" href="{"banners.update?banner_id=`$banner.banner_id`"|fn_url}">{$banner.banner}</a>
                ...
                <td width="10%" class="right" data-th="{__("status")}">
                    {hook name="banners:status"}
                        {include file="common/select_popup.tpl" type="banners" id=$banner.banner_id status=$banner.status hidden=true object_id_name="banner_id" table="banners" popup_additional_class="`$no_hide_input` dropleft"}
                    {/hook}
                </td>
            </tr>
            {/foreach}
...
{capture name="adv_buttons"}
    ...
    {include file="common/tools.tpl"
        tool_href="banners.add"
        tool_override_meta="btn btn-primary"
        prefix="top"
        hide_tools="true"
        title=__("add_banner")
        link_text=__("add_banner")
        icon="icon-plus"
    }
    {/hook}
{/capture}
