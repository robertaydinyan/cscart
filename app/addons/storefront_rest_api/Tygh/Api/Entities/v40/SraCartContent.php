...
class SraCartContent extends ASraEntity
{
        ...
        if (!isset($this->carts[$cart_service_id])) {
            ...
            foreach ($cart['products'] as $product) {
                if (isset($product['chosen_shipping'])) {
                    $cart['chosen_shipping'][$product['group_id']] = $product['chosen_shipping'];
    ...
    protected function calculate(array $cart, array $params = [], $lang_code = DEFAULT_LANGUAGE)
    {
        ...
        // add payment methods
        $cart['payments'] = $this->getPayments($cart, $params, $lang_code);
     ...
     * @return array<string, int|string|array>
     */
    protected function getPayments(array $cart, $params = [], $lang_code = DEFAULT_LANGUAGE)
    {
        $payment_methods = fn_prepare_checkout_payment_methods($cart, $this->auth, $lang_code, false);

        $filtered_payment_methods = array_map(
            function ($payment) {
        ...
        /** @var array{payments_as_list ?: string} $params */
        return !empty($params['payments_as_list']) && YesNo::toBool($params['payments_as_list'])
            ? array_values($filtered_payment_methods)
